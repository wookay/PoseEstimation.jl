...
def netforward(param, model, tnet, image, debug=True):
    import cv2 as cv
    import numpy as np
    import time
...
def convert(currentFrame, oriImg, multiplier, heatmap, paf, jcolors, format='image'):
    import cv2 as cv
    import json
    info = paf_info(currentFrame, oriImg, multiplier, heatmap, paf, jcolors)
    canvas = oriImg
    stickwidth = 4
    for parts in info['body_parts']:
        i = parts['id']
        color = parts['color']
        subset = parts['subset']
        for subs in subset:
...
def paf_info(currentFrame, oriImg, multiplier, heatmap, paf, jcolors):
